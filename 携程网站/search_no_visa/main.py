...
class No_Visa:
    def __init__(self):
        ...
        self.sheet = self.wb.add_sheet(sheetname='免签国家')
        titles = ['国家', '可停留天数', '是否可以延期', '费用', '描述']
        for title in titles:
            self.sheet.write(0, titles.index(title), title)
    ...
    def analyse_content(self):
        ...
        for data in json_data:
            ...
            self.sheet.write(row, col, displayedName)
            col += 1
            ...
            self.sheet.write(row, col, data_time)
            col += 1

            PostponedStructDesc = data['PostponedStructDesc']
            self.sheet.write(row, col, PostponedStructDesc.replace('是否可延期：', ''))
            col += 1

            CostStructDesc = data['CostStructDesc']
            self.sheet.write(row, col, CostStructDesc.replace('费用：', ''))
            col += 1

            Remark = data['Remark']
            self.sheet.write(row, col, Remark)
            row += 1
            col = 0
